...
use wgpu::{Backends, SurfaceError};
use winit::{
...
use crate::types::{Instance, Vertex, View};
...
impl State {
    ...
    async fn new(window: Window, view_size: f32) -> Self {
        ...
        let circle_shader = device.create_shader_module(wgpu::include_wgsl!("shaders/circle.wgsl"));
        let line_shader = device.create_shader_module(wgpu::include_wgsl!("shaders/line.wgsl"));
        let view = View::new(Vec2::zero(), view_size, config.width as u16, config.height as u16);
    ...
    fn render(&mut self, gui: &mut dyn FnMut(&egui::Context)) -> Result<(), SurfaceError> {
        self.queue.write_buffer(&self.view_buffer, 0, bytemuck::cast_slice(&[self.view]));
        ...
        let (clipped_primitives, screen_descriptor) =
            self.gui.render(&self.device, &self.queue, &self.window, &mut encoder, gui);
...
pub trait Renderer {
    fn new() -> Self;
    fn input(&mut self, input: &WinitInputHelper, elapsed: &Duration);
    fn render(&mut self, ctx: &mut RenderContext, elapsed: &Duration);
...
pub fn run<R>(window_mode: WindowMode, window_title: &'static str, view_size: f32) where R: Renderer + 'static,
{
    ...
    thread::spawn(move || {
        ...
        let builder = match window_mode {
            ...
            WindowMode::Fullscreen => WindowBuilder::new()
                .with_title(window_title)
